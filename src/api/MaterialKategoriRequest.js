import axios from "axios";
import { API_MATERIAL_KATEGORI_GET } from "../config/api";

export const getMaterialKategoriRequest = async () => {
  // const user = getLocalUser();
  try {
    const response = await axios.get(`${API_MATERIAL_KATEGORI_GET}`, {
      headers: {
        Authorization: `bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NmhGNPK-EpJfPUZMxlhl8JUiB2u4_y9K5ozPiM3uxl4`,
      },
    });
    return response.data.list_data.master_material_category;
  } catch (error) {
    console.error("Error fetching items:", error);
  }
};
