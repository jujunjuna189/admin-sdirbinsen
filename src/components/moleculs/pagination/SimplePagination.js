    ...
    return (
        <div className="flex gap-2">
            ...
            {props.pages?.map((item, index) => {
                ...
                return <div key={index} className={`px-[0.7rem] py-[0.10rem] border rounded-md flex justify-center items-center cursor-pointer ${(props.currentPage) === (index) ? 'border-slate-600 text-slate-600' : 'border-slate-200 text-dark'}`} onClick={() => props.onCallback ? props.onCallback(index) : {}}><span className="text-[12px]">{item.label}</span></div>;
            })}
        </div>
    );
}
