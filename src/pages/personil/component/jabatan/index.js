    ...
    return (
        <div>
            <div className="shadow-none p-4 border rounded-lg">
                <span className="font-semibold text-base">Jabatan</span>
                <div className="p-3">
                    <span>{props?.jabatan?.data?.[props.jabatan_active]?.jabatan ?? 'Tidak ada jabatan'}</span><br />
                    <div className="mt-1 leading-3">
                        <small className="font-medium">Masa Jabatan</small><br />
                        <small>{props?.jabatan?.data?.[props.jabatan_active]?.tmt}</small> - <small>{props?.jabatan?.data?.[props.jabatan_active]?.tst}</small>
                    </div>
                </div>
                <div className="mt-2 px-3">
                    <span className="font-semibold">Riwayat Jabatan</span>
                </div>
                    ...
                    <div className="border-l-2 border-slate-200 px-5 py-2 relative flex items-center">
                        ...
                        <div className="relative flex items-center w-96">
                            ...
                            <div className="border border-dashed rounded-lg p-3 bg-white pr-10 flex-grow flex justify-between items-center gap-3 cursor-pointer">
                                <AddJabatanModal personil_id={props.personil?.id} onSave={() => props.onAdd && props.onAdd()} />
                            </div>
                        </div>
                    </div>
                    {props.jabatan?.data?.map((item, index) => {
                        return (
...
export default JabatanPersonilDetail;