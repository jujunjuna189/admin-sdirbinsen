...
const UpdatePangkatModal = (props) => {
  const ref = useRef();
  ...
  const getPangkat = async () => {
    await getRiwayatPangkatByPersonilDetailRequest({ personil_id: props.personil_id, pangkat_id: props.item?.id }).then((res) => {
      setController({
        pangkat: res.pangkat,
  ...
  const onSave = async () => {
    ...
    await updateRiwayatPangkatByPersonilRequest({ personil_id: props.personil_id, pangkat_id: props.item?.id, body: dataBatch }).then((res) => {
      ...
      <div className={`fixed top-0 bottom-0 left-0 right-0 flex justify-center items-center z-10 ${!isShow && "hidden"}`}>
        ...
        <div className="p-3 border rounded-lg bg-white w-96 z-10">
          <div className="leading-3">
            <span className="text-base font-medium">Ubah Pangkat</span>
            <br />
...
export default UpdatePangkatModal;
