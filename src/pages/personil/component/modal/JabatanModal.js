...
import { getJabatanPersonilRequest } from "../../../../api/JabatanRequest";
import { EmptyData, InputSelect } from "../../../../components";

const JabatanModal = (props) => {
    const ref = useRef();
    const [isShow, setIsShow] = useState(false);
    const [data, setData] = useState([]);

    const getJabatan = async () => {
        await getJabatanPersonilRequest({}).then((res) => {
            setData(res);
        });
    }
    ...
    return (
        <div className="" ref={ref}>
            <div className="cursor-pointer" onClick={() => toogleModal()}>
                ...
                <InputSelect className="mt-1" error={props.error} placeholder={props.value ?? 'Pilih Jabatan...'} />
            </div>
            <div className={`fixed top-0 bottom-0 left-0 right-0 flex justify-center items-center z-10 ${!isShow && 'hidden'}`}>
                ...
                <div className="p-3 border rounded-lg bg-white w-96 z-10">
                    <div className="leading-3">
                        <span className="text-base font-medium">Pilih Jabatan</span><br />
